...
<p>文档适用版本：3.5.3.229 Update 1（最新）</p>\
<h3>HHML 介绍</h3>\
...
<p>在脚本所在目录下创建一个名为“customPath.txt”的文本文档。内容是一行一个路径，就像这样：</p>\
<textarea>\
...
<p><b>搜索自定义路径</b><br>配置名：searchInCustPath<br>默认参数：2<br>在此范围找到的 Java 会被添加到临时环境变量中，以便 HMCL 检测到。</p>\
<p><b>搜索当前目录</b><br>配置名：searchInCD<br>默认参数：2<br>在此范围找到的 Java 会被添加到临时环境变量中，以便 HMCL 检测到。</p>\
...
<p><b>屏蔽部分版本的 Java（只在 checkJava 参数为 2 时生效）</b><br>配置名：verBlackList<br>用法：将你不想使用的版本的版本号（可以多个,每个版本间用英符逗号隔开）写入参数中，在检测版本时会将与其对应的版本屏蔽掉。<br>默认参数：none<br>参数示例：</p>\
<textarea>\
